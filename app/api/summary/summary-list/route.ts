...
export async function GET() {
  try {
    ...
    const summaryList: Array<WithId<SummaryDB>> = await mongoClient.db(DBName.TRIVIA_SPARK_AI).collection(CollectionName.SUMMARY).find<SummaryDB>({ user_id: user?.user?.email }).limit(7).sort({ _id: -1 }).toArray();
